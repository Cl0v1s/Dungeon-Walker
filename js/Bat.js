function Bat(stairTemp,xTemp,yTemp,spawn,idTemp)
{
	Monster.call(this,stairTemp,xTemp,yTemp,idTemp);
	/** Monster's settings **/
	this.name="chauve-souris";
	this.biome="cave";
	this.image="B";
	this.life=20;
	this.launch=0;
...
Bat.prototype.think=function()
{
	...
	else if(bestFriend != undefined && worstEnemy==undefined)
	{
	...
	else if(worstEnemy != undefined && bestFriend != undefined)
	{
		if(!this.searchEffect("grouped") && !this.searchEffect("leader"))
			this.run(worstEnemy.getX(),worstEnemy.getY());
		else
		{
